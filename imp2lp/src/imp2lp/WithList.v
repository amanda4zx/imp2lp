...
| EEmptyList (l : list (string * type))
| EListInsert (r : rexpr) (e : expr)
(* relational algebra *)
...
(* | EFoldLeft (l : expr) (r0 : rexpr) (v acc : string) (r : rexpr) *).

(* ???
CForach e x ?.
Now aim for producing a position * char table to represent the output string
ultimately want to do string offset arithmetic
 *)
...
Section WithMap.
  ...
  | ESetInsert r s =>
      ...
      (inl {| decl_R := aux; decl_sig := attr_tys |} ::
        ...
        next_rel',
        attr_tys)
  | EEmptyList l => ([], next_rel, ("_pos", DNumber) :: map (fun '(s, t) => (s, lower_type t)) l)
  | EListInsert r l =>
      (* out(0, r). out(i+1, r) :- aux(i, r) *)
      let '(r', _) := lower_rexpr [] r in
      let aux := nat_rel next_rel in
      let '(rls, next_rel', attr_tys) := lower_expr aux (S next_rel) l in
      let vs := List.map var_dexpr (mk_vars 0 (List.length attr_tys)) in
      (inl {| decl_R := aux; decl_sig := attr_tys |} ::
         rls ++
         [ inr {| rule_head := {| fact_R := out; fact_args := (fun_dexpr (fn_ZLit 0) []) :: r' |};
                 rule_body := [];
                 rule_prop := [] |};
           inr {| rule_head := {| fact_R := out; fact_args := (fun_dexpr fn_Plus [hd default_dexpr vs; fun_dexpr (fn_ZLit 1) []]) :: tl vs |};
            rule_body := [ {| fact_R := aux; fact_args := vs |} ];
            rule_prop := [] |}],
        next_rel',
        attr_tys)
  ...
  | EProj s x r =>
      ...
      (inl {| decl_R := aux; decl_sig := attr_tys |} ::
        ...
        out_attr_tys)
        (*
  | EFoldLeft l r0 v acc r =>
      let aux := nat_rel next_rel in
      let '(rls, next_rel', attr_tys') := lower_expr aux (S next_rel) l in
      let '(r0', attr_tys0) := lower_rexpr [] r0 in
      let vars := mk_vars 0 (List.length attr_tys') in
      let vars0 := mk_vars (List.length attr_tys') (List.length attr_tys0) in
      let vs := List.map var_dexpr vars in
      let vs0 := List.map var_dexpr vars0 in
      let m := put_attr_bindings (put_attr_bindings [] v attr_tys' vars) acc attr_tys0 vars0 in
      let '(r', attr_tys) := lower_rexpr m r in
      let iters := nat_rel next_rel' in
      (rls ++
         [inr {| rule_head := {| fact_R := iters; fact_args := fun_dexpr (fn_ZLit 0) [] :: r0'|};
                rule_body := [];
                rule_prop := [] |};
          inr {| rule_head :=
                  {| fact_R := iters;
                    fact_args := fun_dexpr fn_Plus [hd default_dexpr vs; fun_dexpr (fn_ZLit 1) []] :: r' |};
                rule_body :=
                  [ {| fact_R := aux; fact_args := vs |};
                    {| fact_R := iters; fact_args := hd default_dexpr vs :: vs0 |} ];
                rule_prop := [] |};
          inr {| rule_head := {| fact_R := out; fact_args := vs0 |};
                rule_body := [ {| fact_R := iters; fact_args := length of l :: vs0|} ];
                rule_prop := [] |}],
      S next_rel',
        attr_tys) *)
  end.
 ...
 *)
  Definition persons_tbl := EListInsert (RRecord [("id", AInt 2); ("name", AString "A"); ("is_female", ABool false)])
                            (EListInsert (RRecord [("id", AInt 0); ("name", AString "B"); ("is_female", ABool true)])
                               (EListInsert (RRecord [("id", AInt 1); ("name", AString "C"); ("is_female", ABool false)])
                                  (EEmptyList [("id", TInt); ("is_female", TBool); ("name", TString)]))).
  ...
  Compute print_compiled_query (nat_rel 0) 1 persons_tbl.
End WithMap.
(* ???
Semantics of this fiat2 and this datalog. Ignore aggregation for now
Fiat2 to depend on Pyrosome after Dustin updates the code *)
