...
Variant rel' : Type :=
  | blk_mut_rel' (b x : nat)
  | blk_blk_rel' (b0 b1 : nat)
  | blk_terminate_rel' (b : nat)
  | mut_rel' (x : nat)
  | blk_rel' (b : nat)
  | terminate_rel'
  | aux_rel' (a : nat)
...
Definition blocks_prog var := blocks_prog (lvar:=rel') (exprvar:=var') (fn:=fn) (aggregator:=False) (var:=var).

Definition compile_init {var} (rls : module) : blocks_prog var (1 + num_muts + num_blks).
(* can return terminate_rel', mut_rel', blk_rel' *)
Definition compile_asgn {var} (b x : nat) (asgn : module) : blocks_prog var 1.
(* return in aux_rel' b (Some x) *)
Definition compile_flow {var} (b : nat) (fl : dflow) : blocks_prog var (1 + num_blks).
(* may return terminate_rel', one or two blk_rel' *) (* or compute the actual number of return rels from fl *)
Definition union_mut {var} (x : nat) : blocks_prog var 1.
(* union aux_rel' b (Some x) for all b, and mut_rel' from initial module (need to rename that to auxiliary rel) *)
Definition union_blk {var} (blks : list dblocks) : blocks_prog var 1.
(* union blk_rel' b *) (* need to compute which blks can precede b *)
Definition union_terminate {var} (blks : list dblocks) : blocks_prog var 1.
(* union terminate_rel' *) (* need to compute which blks can terminate *)
...
(*
Definition lower_module {var} (num_muts num_blks : nat) (rls : module) : blocks_prog var (1 + num_muts + num_blks) :=
  Block [| mk_muts_tup ++ mk_blks_tup |]
*)
