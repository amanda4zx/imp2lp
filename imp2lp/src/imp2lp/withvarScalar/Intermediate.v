...
Record dblock :=
  { asgns : list module; fl : dflow }.

Record dprog :=
  { init : module; blks : list dblock }.
...
Definition rule_impl (ctx : context) (rl : rule) (f : fact) (f_hyps : list fact) :=
  interp_clause ctx rl.(concl) f /\
    Forall2 (interp_clause ctx) rl.(hyps) f_hyps.
...
Definition union_db (db1 db2 : fact -> Prop) :=
  fun f => db1 f \/ db2 f.
